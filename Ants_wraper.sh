...
# #
################################################################################
export ANTS=/home/cobigo/devel/CPP/ANTs/install
###################
# Freesurfer data #
###################
export SUBJECTS_DIR=/home/cobigo/subjects
export SUBJECT=GazzDCS0004mgh_GPU4
# MRI imaging
T1_DICOM=$SUBJECTS_DIR/MEMPRAGE_4e_p2_1mm_isoRMS_12
FIRST_T1_DICOM=IM-0012-0001.dcm
#
T2_DICOM=$SUBJECTS_DIR/T2_SPACE_1mm_iso_15
FIRST_T2_DICOM=IM-0015-0001.dcm
# MRI Diffusion Dat Imaging.
DTI_DICOM=$SUBJECTS_DIR/ep2dadvdiff511E_b2000_64dir_19
...
############################
export LD_LIBRARY_PATH=$ANTS/lib:$MNE_ROOT/lib:$FSL/lib:$LD_LIBRARY_PATH
#
export PATH=$ANTS/bin:$FSL/bin:$NIFTI/install/bin:$MNE_ROOT/bin:$PATH
################################################################################
# #
...
check()
{
    ...
    ## commands
    #    command -v  N4BiasFieldCorrection >/dev/null 2>&1 || { echo >&2 "ANTs - N4 required but it's not installed. Aborting."; exit 1;    
	#command -v matlab >/dev/null 2>&1 || { echo >&2 "Matlab required but it's not installed. Aborting."; exit 1; }
    command -v N4BiasFieldCorrection >/dev/null 2>&1 || \
	{ echo >&2 "ANTs - N4 required but it's not installed. Aborting."; exit 1; }
...
fsl()
{
    ...
    #
    test -s $SUBJECTS_DIR/$SUBJECT/mri/T1.nii && \
	T1=$SUBJECTS_DIR/$SUBJECT/mri/T1.nii \
	|| echo "File T1 not found!";
    #
    bet $T1 $PWD/T1_brain.nii -m -R -f 0.4
    bet $T1 $PWD/T1_skull_stript.nii -n -R -f 0.4
    #
}
################################################################################
# #
...
#	|| echo "File T2 not found!";
    #
    N4BiasFieldCorrection -i T1_brain.nii.gz -x T1_brain_mask.nii.gz -o T1_nubias.nii
}
################################################################################
# #
# FIJEE EXECUTION #
